...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // filteredSample = allSamples.metadata.filter(i => i.id === sample);
    ...
    sortedSample = firstSample1.sort(function(a,b){ return a[3] > b[3] ? 1 : -1; });
    topTenOtuIds = sortedSample[0].otu_ids.slice(0,10);
    topTenSampleValues = sortedSample[0].sample_values.slice(0,10);
    topTenOtuLabels = sortedSample[0].otu_labels.slice(0,10);
    ...
    var barData = {
      x: sortedSample.map(i => i.sample_values).slice(0,10),
      y: sortedSample.map(i => i.otu_ids).slice(0,10),
      text: sortedSample.map(i => i.otu_labels).slice(0,10),
      name: "NJAIN",
      type: "bar",
      orientation:'h',
    };
